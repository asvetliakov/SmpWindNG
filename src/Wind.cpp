...
void wind::Wind::onEvent(const hdt::PreStepEvent& e) {
    ...
    m_sky = Sky::GetSingleton();
    if (m_sky && m_sky->mode == Sky::Mode::kFull && m_sky->windSpeed != 0.0f) {
        ...
        // The overhead of waking up the threads is bigger than the time save, unless there are a lot of objects to
        // process. This is likely to be situational, though. Potentially, this could be tuned automatically by
        // alternating between the paths and favouring whichever is currently faster. Probably not worth the trouble,
        // though.
...
btVector3 wind::Wind::eval(const btVector3& at) {
    ...
    if (speed > 0.0f) {
        ...
        float phase01 =
            m_config->getf(Config::OSC01FREQ) * m_currentTime - U / m_config->getf(Config::OSC01FREQ) - V - W;
        float phase02 =
            m_config->getf(Config::OSC02FREQ) * m_currentTime - U / m_config->getf(Config::OSC02FREQ) - V - W;

        // Two different frequencies makes fluctuations less predictable
        btVector3 base =
            (1.0f + 0.5f * m_config->getf(Config::OSC01FORCE) * (std::cosf(phase01) + std::cosf(0.2692f * phase01))) *
            m_currentDir;

        float angle = m_sky->windAngle +
                      0.5f * m_config->getf(Config::OSC02SPAN) * (std::cosf(phase02) + std::cosf(0.3101f * phase02));
        ...
        btVector3 noise =
            m_config->getf(Config::NOISE) * btVector3(std::fmodf(10.0f * at[0], 2.0f) - std::copysignf(1.0f, at[0]),
